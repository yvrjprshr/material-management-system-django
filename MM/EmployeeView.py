...
def CheckEmployeeLogin(request):
   try:
    ...
    if(result):
        ...
        return render(request, "EmployeeLogin.html", {'result': result,'msg':'Invalid/Email id and password'})
    db.close()
   except Exception as e:
       print("Error!!",e)
...
def EmployeeSubmit(request):
    try:
       ...
       for chunk in picture.chunks():
           F.write(chunk)
       F.close()
       result = EmailService.SendHTMLMail(emailaddress,("Welcome ,Hii {} Your Password is {}".format(firstname, password)))

       db.close()
       result=SendSms.SendMessage("Welcome ,Hii {} Your Password is {}".format(firstname,password),mobilenumber)
       print(result.json())
       return render(request, "EmployeeInterface.html",{'msg':'Record Successfully Submitted'})

    except Exception as e:
        print("Error:",e)
        return render(request, "EmployeeInterface.html",{'msg':'Record Submission Failed'})
